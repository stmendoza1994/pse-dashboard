...
/* CONTENT */
.content { padding: 0rem 7rem 3.5rem 7rem; }
...
col_indent, col_about, col_sector, col_vc, col_space = st.columns([0.5, 0.8, 1.2, 1.2, 1.3])
with col_about:
...
for col, label, val, color in [
    ...
    col.markdown(f"""
    <div style='background:#ffffff;border-radius:8px;padding:0.2rem 0.4rem;text-align:center;border:1px solid #b8d4e4;
...
if page == "Dashboard":
    ...
    table_html = f"""
    <div style='overflow-x:auto;overflow-y:auto;max-height:420px;max-width:100%;border-radius:14px;border:1px solid #b8d4e4;
