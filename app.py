...
[data-testid="stSidebar"] p { color: #6a96ae !important; font-size: 0.85rem !important; }
[data-testid="stSidebar"] [data-testid="stToggle"] label {
    font-size: 0.92rem !important; color: #c0d8e4 !important;
}
...
.bstat-label {
    ...
    text-transform: uppercase; color: #6a96ae; margin-bottom: 0.35rem; line-height: 1;
}
.bstat-val {
...
.card-label {
    ...
    letter-spacing: 0.1em; color: #8a9faf; margin-bottom: 0.65rem; line-height: 1;
}
.card-val {
...
[data-testid="stDataFrame"] {
    ...
    box-shadow: 0 1px 4px rgba(0,0,0,0.06), 0 2px 10px rgba(0,0,0,0.04) !important;
    border: none !important;
}
[data-testid="stDataFrame"] table { background: #fff !important; }
[data-testid="stDataFrame"] thead tr th {
    background: #f5f6fa !important; color: #8a9faf !important;
    font-size: 0.75rem !important; font-weight: 700 !important;
    letter-spacing: 0.06em !important; text-transform: uppercase !important;
    border-bottom: 1px solid #e2e6ed !important;
}
[data-testid="stDataFrame"] tbody tr td {
    color: #1a2e3b !important; font-size: 0.9rem !important;
    border-bottom: 1px solid #f5f6fa !important;
}
[data-testid="stDataFrame"] tbody tr:hover td { background: #f8f9fc !important; }

/* ── DETAIL ROWS ── */
.drow {
    display: flex; justify-content: space-between; align-items: center;
    padding: 0.75rem 0; border-bottom: 1px solid #f0f3f6;
    font-size: 0.92rem; line-height: 1.4; letter-spacing: 0.01em;
}
...
.badge-g {
    ...
    padding: 4px 14px; font-size: 0.78rem; font-weight: 700; letter-spacing: 0.03em;
}
.badge-n {
...
.bkdown {
    ...
    font-size: 0.82rem; color: #2c3e50; white-space: pre-wrap;
    line-height: 1.9; max-height: 560px; overflow-y: auto; letter-spacing: 0.02em;
}
...
[data-testid="stExpander"] summary {
    ...
    color: #1a2e3b !important; letter-spacing: 0.01em !important;
    padding: 1.1rem 1.4rem !important;
}
[data-testid="stExpander"] summary:hover { background: #f8f9fc !important; }

/* ── SELECTBOX ── */
[data-testid="stSelectbox"] > div > div {
    background: #fff; border: 1px solid #dde3ec;
    border-radius: 10px; font-size: 0.95rem; letter-spacing: 0.01em;
    color: #1a2e3b;
}
...
def robust_mean(series):
    """Mean excluding NaN and IQR outliers."""
    s = series.dropna()
...
with st.sidebar:
    ...
    <div style='margin-bottom:2rem;padding-bottom:1.4rem;border-bottom:1px solid #2a4f68;'>
        ...
        <div style='font-size:0.8rem;color:#6a96ae;margin-top:0.4rem;
        letter-spacing:0.03em;'>Fundamental Screener</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style='margin-top:2rem;padding-top:1.4rem;border-top:1px solid #2a4f68;'>
        ...
        <div style='display:flex;justify-content:space-between;margin-bottom:0.8rem;'>
            <span style='font-size:0.9rem;color:#6a96ae;'>Showing</span>
            <span style='font-size:0.9rem;font-weight:700;color:#fff;'>{len(fdf)} of {len(df)}</span>
        </div>
        <div style='display:flex;justify-content:space-between;margin-bottom:0.8rem;'>
            <span style='font-size:0.9rem;color:#6a96ae;'>With data</span>
            <span style='font-size:0.9rem;font-weight:700;color:#5ac8fa;'>{len(valid)}</span>
        </div>
        <div style='display:flex;justify-content:space-between;'>
            <span style='font-size:0.9rem;color:#6a96ae;'>Value creators</span>
            <span style='font-size:0.9rem;font-weight:700;color:#4cd964;'>{creators}</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
# ── Banner ────────────────────────────────────────────────────────────────────
avg_roic   = robust_mean(valid["roic_pct"])   if len(valid) else None
avg_wacc   = robust_mean(valid["wacc_pct"])   if len(valid) else None
...
with k1:
    ...
    st.markdown(f"""<div class="card">
        ...
        <div class="card-val {fc}">{money(fv)}</div>
...
with k4:
    ...
    # Purely descriptive — no value judgment labels
    if sp is not None:
        bdg = f"<span class='badge-{'g' if is_vc else 'n'}'>ROIC {'above' if is_vc else 'below'} WACC</span>"
    else:
        bdg = ""
    st.markdown(f"""<div class="card">
...
with right:
    ...
    fig1 = go.Figure(go.Bar(
        ...
        textfont=dict(size=12, color="#1a2e3b", family="Inter"), width=0.42,
    ))
    ...
    fig2 = go.Figure(go.Bar(
        ...
        textfont=dict(size=13, color="#1a2e3b", family="Inter"), width=0.32,
    ))
    fig2.update_layout(
        paper_bgcolor="#fff", plot_bgcolor="#fff",
        font=dict(family="Inter", color="#8a9faf", size=12),
        ...
        showlegend=False, height=215,
    )

    # Both charts in one card, tight together
    st.markdown('<div class="card" style="padding:1.3rem 1.5rem;">', unsafe_allow_html=True)
    st.plotly_chart(fig1, use_container_width=True, config={"displayModeBar": False})
    st.markdown('<hr style="border:none;border-top:1px solid #f0f3f6;margin:0.5rem 0;">', unsafe_allow_html=True)
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown("""
<div style='background:#1d3a4f;padding:1.3rem 3rem;margin-top:3rem;
